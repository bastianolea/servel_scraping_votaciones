...
tabla <- datos_tabla |> 
  ...
  tab_header(title = md("_Resultados parciales:_ Elecciones Municipales 2024"),
             subtitle = md(glue("Alcaldes de la Región Metropolitana con mayoría de votos al momento"))) |> 
  # formato de números
  fmt_percent(c(porcentaje, mesas_porcentaje), decimals = 1) |> 
  fmt_number(votos, 
...
# guardar ----
gtsave(tabla, 
       filename = glue("tablas/servel_tabla_ganadores_rm_{formatear_fecha(fecha_scraping)}.png"),
       quiet = TRUE) |> 
  suppressWarnings()

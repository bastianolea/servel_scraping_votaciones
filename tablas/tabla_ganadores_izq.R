...
datos_tabla_2 <- datos_tabla |> 
  # select(1:5) |> 
  left_join(
    cut_comunas |> 
      mutate(comuna_match = stringi::stri_trans_general(comuna, "latin-ascii"),
             comuna_match = toupper(comuna_match),
             comuna_match = recode(comuna_match, 
                                   "VINA DEL MAR" = "VIÑA DEL MAR",
                                   "PENALOLEN" = "PEÑALOLEN")), 
                          by = c("comuna" = "comuna_match")) |> 
  rename(comuna_original = comuna,
         comuna = comuna.y) |> 
...
tabla_ganadores_izq <- datos_tabla_2 |>
  ungroup() |> 
  arrange(desc(votos)) |>
  select(comuna, region, candidato, partido, votos, porcentaje, mesas_porcentaje, sector) |> 
  gt() |> 
  tab_header(title = md("**Resultados:** Elecciones Municipales 2024"),
             subtitle = md(glue("Alcaldes electos de izquierda"))) |> 
  # formato de números
...
# guardar ----
gtsave(tabla_ganadores_izq, 
       filename = glue("tablas/resultados/servel_tabla_ganadores_izq_{formatear_fecha(fecha_scraping)}.png"),
       quiet = TRUE) |> 
  suppressWarnings()

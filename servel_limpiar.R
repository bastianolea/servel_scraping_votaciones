...
tabla_5 <- tabla_4 |> 
  mutate(tipo_totales = ifelse(lista_pacto %in% c("Válidamente Emitidos", 
                                             "Votos Nulos", 
                                             "Votos en Blanco"), TRUE, FALSE)) |> 
  
  mutate(tipo_pacto = ifelse(lista_pacto |> str_detect("^.-\\w+"), TRUE, FALSE)) |> 
  # corregir cifras
